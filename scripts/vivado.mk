...
# IP_TCL_FILES - space-separated list of IP TCL files
# BD_TCL_FILES - space-separated list of Block Design TCL files
#
# Example:
#
# FPGA_TOP = fpga
...
BD_TCL_FILES_REL = $(foreach p,$(BD_TCL_FILES),$(if $(filter /% ./%,$p),$p,../$p))
...
create_project.tcl: Makefile $(XCI_FILES_REL) $(IP_TCL_FILES_REL) $(BD_TCL_FILES_REL) $(CONFIG_TCL_FILES_REL)
	...
	for x in $(BD_TCL_FILES_REL); do echo "source $$x" >> $@; done
	echo "make_wrapper -files [get_files design_1.bd] -top -import" >> $@
...
$(PROJECT).runs/synth_1/$(PROJECT).dcp: create_project.tcl update_config.tcl $(SYN_FILES_REL) $(INC_FILES_REL) $(XDC_FILES_REL) | $(PROJECT).xpr
	...
	echo "launch_runs -jobs 3 synth_1" >> run_synth.tcl
...
$(PROJECT).runs/impl_1/$(PROJECT)_routed.dcp: $(PROJECT).runs/synth_1/$(PROJECT).dcp
	...
	echo "launch_runs -jobs 3 impl_1" >> run_impl.tcl
...
# pdi file
$(PROJECT).pdi $(PROJECT).ltx: $(PROJECT).runs/impl_1/$(PROJECT)_routed.dcp
	echo "open_project $(PROJECT).xpr" > generate_pdi.tcl
	echo "open_run impl_1" >> generate_pdi.tcl
	echo "write_device_image -force $(PROJECT).runs/impl_1/$(PROJECT).pdi" >> generate_pdi.tcl
	if [ -e $(PROJECT).runs/impl_1/$(PROJECT).ltx ]; then \
		echo "write_debug_probes -force $(PROJECT).runs/impl_1/$(PROJECT).ltx" >> generate_pdi.tcl; \
	fi
	vivado -nojournal -nolog -mode batch -source generate_pdi.tcl
	ln -f -s $(PROJECT).runs/impl_1/$(PROJECT).pdi .
	if [ -e $(PROJECT).runs/impl_1/$(PROJECT).ltx ]; then ln -f -s $(PROJECT).runs/impl_1/$(PROJECT).ltx .; fi
	mkdir -p rev
	COUNT=100; \
	while [ -e rev/$(PROJECT)_rev$$COUNT.pdi ]; \
	do COUNT=$$((COUNT+1)); done; \
	cp -pv $(PROJECT).runs/impl_1/$(PROJECT).pdi rev/$(PROJECT)_rev$$COUNT.pdi; \
